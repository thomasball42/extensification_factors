...
and yield (intensive) margins, for a single area+item.
...
assert len(area_matches) == 1, (
    f"afilt/aexcl must resolve to exactly one Area, got {area_matches}"
)
area = area_matches[0]
...
d_area, d_yield, d_prod, years = d_area[valid], d_yield[valid], d_prod[valid], years[valid]


# holds exactly in theory (Production = Area harvested * Yield), but FAOSTAT
# rounds Area/Yield/Production independently, so allow a little slack
residual = np.abs((d_area + d_yield) - d_prod)
assert residual.max() < 0.5, (
    f"log-diff decomposition should be ~additive, max residual = {residual.max():.3f} pct points"
)
...
ax.set_title(f"{item} — {area}: area vs yield contribution to production growth")
...
size_handles = [
    ax.scatter([], [], s=20 + size_k * v, color="0.5", edgecolor="0.3", linewidth=0.3)
    for v in size_ref_vals
]
ax.legend(size_handles, [f"{v}% ΔProduction" for v in size_ref_vals],
...
if SAVE:
    filename = f"area_yield_decomposition_{item}_{area}.png"
